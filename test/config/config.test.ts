...
describe('Configuration System', () => {
  ...
  describe('devConfig', () => {
    const devConfig = getDevConfig(testAccountId, testRegion, TEST_EXTERNAL_VALUES);
  ...
  describe('getConfig', () => {
    test('returns dev config for dev environment', () => {
      const config = getConfig('dev', testAccountId, testRegion, TEST_EXTERNAL_VALUES);
      expect(config.environment).toBe('dev');
    });

    test('returns staging config for staging environment', () => {
      const config = getConfig('staging', testAccountId, testRegion, TEST_EXTERNAL_VALUES);
      expect(config.environment).toBe('staging');
    });

    test('returns production config for production environment', () => {
      const config = getConfig('production', testAccountId, testRegion, TEST_EXTERNAL_VALUES);
