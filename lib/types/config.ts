...
/**
 * External values provided via CDK context or environment variables.
 *
 * These are org-specific values that should not be hardcoded in config files,
 * allowing the codebase to be shared or open-sourced.
 */
export interface ExternalValues {
  /** Route53 hosted zone ID */
  readonly hostedZoneId?: string;

  /** Domain name (e.g., 'devops.example.com') */
  readonly domainName?: string;

  /** Admin IAM role ARN for cluster access */
  readonly adminRoleArn?: string;

  /** GitOps repository URL for ArgoCD */
  readonly gitOpsRepoUrl?: string;
